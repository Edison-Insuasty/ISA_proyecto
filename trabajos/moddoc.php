...
<body>
    ...
    <div class="card col-10 " style="background-color: #F0F2EE;">
      <div data-bs-spy="scroll" data-bs-target="#navbar-example2" data-bs-root-margin="0px 0px -40%" data-bs-smooth-scroll="true" class="scrollspy-example bg-transparent p-3 rounded-2" tabindex="0">
       <?php
       $cod = $_POST["codigo"];
       $con = "SELECT Cedula, Especializacion FROM docente WHERE CodigoDocente = '$cod' ";
       $con = mysqli_query($conexion,$con);
       if($con){
         while ($row1 = $con->fetch_array()){
          $ced = $row1['Cedula'];
          $esp = $row1['Especializacion'];
       ...
       $con2 = "SELECT CodigoDocente FROM docente WHERE CodigoDocente = '$cod' ";
       $con2 = mysqli_query($conexion,$con2);
       $con2 = mysqli_fetch_array($con2);
       if ($con2) {
        ?>
        <form class="row g-3" name="modificar estudiante" method="POST" action="php/ModificarDocenteAdmin.php">
          <table class="table table-success table-striped">
            ...
            <tr align="center" valign="middle">
              ...
              <td>
                <input type="text" name="especializacion" placeholder="Especialización" class="form-control" value="<?php echo $esp ?>" id="validationDefault05" required>
              </td>
              <td>
                Contraseña:
              </td>
